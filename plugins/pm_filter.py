...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    return await query.answer('♥️ Love @janapraveen ♥️')
            else:
                await query.message.edit_text(
                    "I'm not connected to any groups!\nCheck /connections or connect to any groups",
                    quote=True
                )
                return await query.answer('♥️ Thank You Jana ♥️')
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if (st.status == enums.ChatMemberStatus.OWNER) or (str(userid) in ADMINS):
                ...
                await query.answer("That's not for you Nanba!", show_alert=True)
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("Don't be too smart nanba😒", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        await query.answer('♥️ Thank You Jana ♥️')
